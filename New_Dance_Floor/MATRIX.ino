...
uint8_t modeNumber, colourNumber;
...
void MatrixSetMode(uint8_t Val) {
  modeNumber = Val;
}
void MatrixSetColour(uint8_t Val) {
  colourNumber = Val;
}

void MatrixSetColours(uint8_t r, uint8_t g, uint8_t b) {
  Red = r;
  Green = g;
  Blue = b;
}

uint8_t MatrixGetMode() {
  return modeNumber;
}
uint8_t MatrixGetColour() {
  return colourNumber;
}
